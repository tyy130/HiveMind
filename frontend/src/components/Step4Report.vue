                    ...
                    <div class="result-wrapper" :class="'result-' + log.details?.tool_name">
                      ...
                      <div v-if="!showRawResult[log.timestamp]" class="result-structured">
                        ...
                        <template v-if="log.details?.tool_name === 'interview_agents'">
                          <InterviewDisplay :result="parseInterviewContract(log.details.result)" :result-length="log.details?.result_length" />
                        </template>

                        <template v-else-if="log.details?.tool_name === 'insight_forge'">
                          <InsightDisplay :result="parseInsightForgeContract(log.details.result)" :result-length="log.details?.result_length" />
                        </template>

                        <template v-else-if="log.details?.tool_name === 'panorama_search'">
                          <PanoramaDisplay :result="parsePanoramaContract(log.details.result)" :result-length="log.details?.result_length" />
                        </template>

                        <template v-else-if="log.details?.tool_name === 'quick_search'">
                          <QuickSearchDisplay :result="parseQuickSearchContract(log.details.result)" :result-length="log.details?.result_length" />
                        </template>
...
import {
  parseInsightForge as parseInsightForgeContract,
  parseInterview as parseInterviewContract,
  parsePanorama as parsePanoramaContract,
  parseQuickSearch as parseQuickSearchContract
} from '../utils/reportParsers.js'
