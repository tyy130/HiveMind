...
Run simultaneously Twitter and Reddit Simulation,Read the same configuration file
...
- Support through IPC receive Interview command
- Support single Agent Interviews and batch interviews
...
# ============================================================
# solve Windows Encoding issues:at all import previously set UTF-8 encoding
...
if sys.platform == 'win32':
    # settings Python Default I/O encoded as UTF-8
    # This affects all unspecified encodings open() call
    os.environ.setdefault('PYTHONUTF8', '1')
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')

    # Reconfigure standard output as UTF-8 to prevent garbled Unicode text.
    ...
    # Note:This needs to be in Python Set at startup,Runtime settings may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        packaging open() function,For text mode the default is to use UTF-8 encoding
...
# Load the project root directory .env File(contains LLM_API_KEY and other configurations)
...
def disable_oasis_logging():
    ...
    for logger_name in oasis_loggers:
        ...
        logger.setLevel(logging.CRITICAL)  # Only log serious errors
...
def init_logging_for_simulation(simulation_dir: str):
    ...
    # Clean out the old ones log Directory(if exists)
...
# Twitter Available actions(Not included INTERVIEW,INTERVIEW can only pass ManualAction manual trigger)
TWITTER_ACTIONS = [
...
# Reddit Available actions(Not included INTERVIEW,INTERVIEW can only pass ManualAction manual trigger)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    Manage environments for both platforms,Process Interview command
    """
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get the environment and environment of the specified platform agent_graph
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        """
        Execute on a single platform Interview

        Returns:
            a dictionary containing the results,or contain error dictionary
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        """
        Process a single Agent interview order
        ...
        if success_count > 0:
            ...
            print(f"  Interview Complete: agent_id={agent_id}, Number of successful platforms={success_count}/{len(platforms_to_interview)}")
            return True
        else:
            errors = [f"{p}: {r.get('error', 'unknown error')}" for p, r in results["platforms"].items()]
            self.send_response(command_id, "failed", error="; ".join(errors))
            print(f"  Interview failed: agent_id={agent_id}, All platforms fail")
            return False
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform(can be used by each interview item coverage)
                - "twitter": Interview only Twitter Platform
                - "reddit": Interview only Reddit Platform
                - None/Not specified: each Agent Interview on two platforms at the same time
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        ...
        # put both_platforms_interviews Split into two platforms
        ...
        if results:
            ...
            print(f"  batch Interview Complete: {len(results)} a Agent")
            return True
        else:
    ...
    def _get_interview_result(self, agent_id: int, platform: str) -> Dict[str, Any]:
        """Get the latest from the database Interview result"""
        ...
        try:
            ...
            print(f"  read Interview result failed: {e}")
...
def _get_post_info(
    ...
    Returns:
        contains content and author_name dictionary,or None
    """
    try:
        cursor.execute("""
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    if use_boost and has_boost_config:
        ...
        config_label = "[speed up LLM]"
    else:
        # Use common configuration
        llm_api_key = os.environ.get("LLM_API_KEY", "")
        llm_base_url = os.environ.get("LLM_BASE_URL", "")
        llm_model = os.environ.get("LLM_MODEL_NAME", "")
        config_label = "[Universal LLM]"

    # if .env There is no model name in,then use config as a backup
    if not llm_model:
        llm_model = config.get("llm_model", "gpt-4o-mini")

    # settings camel-ai Required environment variables
    if llm_api_key:
        os.environ["OPENAI_API_KEY"] = llm_api_key

    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("missing API Key Configuration,Please go to the project root directory .env Set in file LLM_API_KEY")
...
def get_active_agents_for_round(
    ...
    """Decide which ones to activate this round based on time and configuration Agent"""
...
async def run_twitter_simulation(
    ...
    Returns:
        PlatformSimulation: contains env and agent_graph the result object
    """
    ...
    # Twitter Use generic LLM Configuration
    model = create_model(config, use_boost=False)

    # OASIS Twitter use CSV Format
    ...
    for round_num in range(total_rounds):
        ...
        # whether active or not agent,All recorded round start
        if action_logger:
            action_logger.log_round_start(round_num + 1, simulated_hour)

        if not active_agents:
            # Not active agent also recorded round end(actions_count=0)
            if action_logger:
    ...
    # Note:Do not close the environment,reserved for Interview use
    ...
    log_info(f"Simulation loop complete! Time consuming: {elapsed:.1f}seconds, total action: {total_actions}")
...
async def main():
    ...
    parser.add_argument(
        '--twitter-only',
        action='store_true',
        help='Just run Twitter Simulation'
    )
    parser.add_argument(
        '--reddit-only',
        action='store_true',
    ...
    # in main Created at the beginning of the function shutdown event,Ensure that the entire program responds to exit signals
    ...
    log_manager.info(f"  - Agent Quantity: {len(config.get('agent_configs', []))}")

    log_manager.info("Log structure:")
    log_manager.info(f"  - main log: simulation.log")
    log_manager.info(f"  - Twitter action: twitter/actions.jsonl")
    log_manager.info(f"  - Reddit action: reddit/actions.jsonl")
    log_manager.info("=" * 60)
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop complete! Total time spent: {total_elapsed:.1f}seconds")
...
def setup_signal_handlers(loop=None):
    ...
    Persistence simulation scenario:Do not exit after simulation is completed,wait interview command
    When a termination signal is received,need:
    1. Notification asyncio loop exit wait
