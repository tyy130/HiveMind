...
management Twitter and Reddit Dual platform parallel simulation
...
class SimulationState:
    ...
    def get_default_platform(self) -> str:
        """Return to default platform based on enabled status"""
    ...
    def to_simple_dict(self) -> Dict[str, Any]:
        """Simplified status dictionary(API Return to use)"""
        return {
            "simulation_id": self.simulation_id,
            "project_id": self.project_id,
            "graph_id": self.graph_id,
...
class SimulationManager:
    ...
    Core functions:
    1. from Zep Graph reads entities and filters
    2. generate OASIS Agent Profile
    3. use LLM Intelligent generation of simulation configuration parameters
    ...
    def prepare_simulation(
        ...
        steps:
        1. from Zep Graph reads and filters entities
        2. generated for each entity OASIS Agent Profile(Optional LLM Enhance,Support parallelism)
        3. use LLM Intelligent generation of simulation configuration parameters(time,Activity,Speech frequency etc)
        4. Save the configuration file and Profile File
        5. Copy the preset script to the simulation directory

        Args:
            simulation_id: Simulation ID
            simulation_requirement: Simulation requirement description(used for LLM Generate configuration)
            document_text: Original document content(used for LLM Understand the context)
            defined_entity_types: Predefined entity types(Optional)
            use_llm_for_profiles: Whether to use LLM Generate detailed persona
            progress_callback: Progress callback function (stage, progress, message)
            parallel_profile_count: The number of characters generated in parallel,Default 3
        ...
        try:
            ...
            # incoming graph_id to enable Zep Search function,Get richer context
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # incoming graph_id used for Zep Search
            ...
            # save Profile File(Note:Twitter use CSV Format,Reddit use JSON Format)
            ...
            if state.enable_twitter:
                # Twitter use CSV Format!This is OASIS requirements
                generator.save_profiles(
                    profiles=profiles,
            ...
            # ========== stage 3: LLM Intelligent generation of simulation configurations ==========
            if progress_callback:
                progress_callback(
                    "generating_config", 0,
            ...
            # Note:The run script remains in backend/scripts/ Directory,No longer copied to mock directory
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                ...
                f"2. Run simulation (The script is located at {scripts_dir}):\n"
                f"   - Run alone Twitter: python {scripts_dir}/run_twitter_simulation.py --config {config_path}\n"
                f"   - Run alone Reddit: python {scripts_dir}/run_reddit_simulation.py --config {config_path}\n"
                f"   - Running dual platforms in parallel: python {scripts_dir}/run_parallel_simulation.py --config {config_path}"
            )
        }
