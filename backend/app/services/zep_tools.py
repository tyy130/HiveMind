...
Package map search,Node read,Edge query and other tools,supply Report Agent use

Core search tools(After optimization):
1. InsightForge(Deep insight search)- The most powerful hybrid search,Automatically generate sub-questions and search in multiple dimensions
...
class SearchResult:
    ...
    def to_text(self) -> str:
        """Convert search results to a report-friendly text format."""
        text_parts = [
            "### Search Query",
            self.query,
        ...
        if self.edges:
            ...
            for i, edge in enumerate(self.edges, 1):
                text_parts.append(f"{i}. {json.dumps(edge, ensure_ascii=False, default=str)}")

        if self.nodes:
            text_parts.append("\n### Related Nodes")
            for i, node in enumerate(self.nodes, 1):
                text_parts.append(f"{i}. {json.dumps(node, ensure_ascii=False, default=str)}")
...
class EdgeInfo:
    ...
    def to_text(self, include_temporal: bool = False) -> str:
        ...
        if include_temporal:
            ...
            invalid_at = self.invalid_at or "so far"
...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        """Convert insights to a report-friendly text format."""
        text_parts = [
            "## Insight Forge",
            "### Analysis Question",
            self.query,
...
class PanoramaResult:
    ...
    Contains all relevant information,Include expired content
    """
    query: str
    ...
    # Expired/fact of failure(History)
    ...
    def to_text(self) -> str:
        """Convert panorama results to a complete report-friendly format."""
        text_parts = [
            "## Panorama Search",
            "### Search Query",
            self.query,
        ...
        # key entity(full output,Do not truncate)
        if self.all_nodes:
            text_parts.append("\n### Core Entities")
...
class AgentInterview:
    """single Agent interview results"""
    agent_name: str
    ...
    def to_text(self) -> str:
        ...
        # show complete agent_bio,Do not truncate
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content(Truncate by period,rather than hard truncation)
...
class InterviewResult:
    ...
    Contains multiple simulations Agent interview answers
    """
    ...
    def to_text(self) -> str:
        """Convert interviews to a detailed report-friendly format."""
        text_parts = [
            "## In-depth Interview Report",
            "### Interview Topic",
            self.interview_topic,
            f"Interviewed {self.interviewed_count} of {self.total_agents} agents.",
            "\n### Selection Rationale",
...
class ZepToolsService:
    ...
    [Core search tools - After optimization]
    1. insight_forge - Deep insight search(The most powerful,Automatically generate sub-questions,Multidimensional search)
    2. panorama_search - breadth search(Get the full picture,Include expired content)
    3. quick_search - Simple search(Quick search)
    4. interview_agents - in-depth interview(Interview simulation Agent,Get multiple perspectives)

    [basic tools]
    - search_graph - Graph semantic search
    - get_all_nodes - Get all nodes in the graph
    - get_all_edges - Get all edges of the graph(Contains time information)
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        # LLM client for InsightForge Generate subproblems
        self._llm_client = llm_client
        logger.info(t("console.zepToolsInitialized"))

    @property
    def llm(self) -> LLMClient:
        """Lazy initialization LLM client"""
    ...
    def search_graph(
        ...
        Use hybrid search(Semantics+BM25)Search for relevant information in the graph.
        if Zep Cloud of search API Not available,then downgrade to local keyword matching.

        Args:
            graph_id: Atlas ID (Standalone Graph)
            query: search query
    ...
    def _local_search(
        ...
        """
        Local keyword matching search(as Zep Search API downgrade plan)

        Get all edges/node,Then perform keyword matching locally
        ...
        # Extract query keywords(simple participle)
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        Returns:
            edge list(contains created_at, valid_at, invalid_at, expired_at)
        """
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        By getting all the edges of the graph,Then filter out the edges related to the specified node
    ...
    def get_entities_by_type(
        ...
        filtered = []
        for node in all_nodes:
            # Check labels Whether the specified type is included
    ...
    def get_simulation_context(
        ...
        # Filter entities with actual types(Impure Entity node)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        The most powerful hybrid search function,Automatically decompose questions and search in multiple dimensions:
        1. use LLM Break the problem into sub-problems
        ...
        5. Consolidate all results,Generate deep insights
        ...
        # Step 1: use LLM Generate subproblems
        ...
        # Step 3: Extract related entities from edges UUID,Get only information about these entities(Do not get all nodes)
        entity_uuids = set()
        ...
        # Get details of all related entities(No limit on quantity,full output)
        entity_insights = []
...
request:
1. Each sub-question should be specific enough,Related ones can be found in Simulation World Agent behavior or event
2. Subproblems should cover different dimensions of the original problem(Such as:who,what,why,How is it,when,Where)
3. Sub-problems should be relevant to the simulation scenario
4. Return JSON Format:{"sub_queries": ["subproblem 1", "subproblem 2", ...]}"""
...
Return JSON List of sub-questions in format."""
    ...
    def panorama_search(
        ...
        Args:
            ...
            include_expired: Does it contain expired content(Default True)
    ...
    def quick_search(
        ...
        1. call directly Zep Semantic search
        ...
        # Directly call an existing search_graph method
    ...
    def interview_agents(
        ...
        call real OASIS interview API,Interview simulation running Agent:
        1. Automatically read character files,Learn about all simulations Agent
        2. use LLM Analyze interview needs,Intelligent selection of the most relevant Agent
        3. use LLM Generate interview questions
        ...
        [important]This feature requires the simulation environment to be running(OASIS The environment is not closed)
        ...
        - Need to get simulation Agent real answer(Not LLM Simulation)

        Args:
            simulation_id: Simulation ID(Used to locate persona files and call interviews API)
            interview_requirement: Interview requirement description(unstructured,Such as"Understand students views on the incident")
            simulation_requirement: Simulation requirements background(Optional)
            max_agents: Most interviewed Agent Quantity
        ...
        if not profiles:
            ...
            result.summary = "No interviews found Agent Persona file"
            return result

        result.total_agents = len(profiles)
        logger.info(t("console.loadedProfiles", count=len(profiles)))

        # Step 2: use LLM Choose to interview Agent(Return agent_id list)
        ...
        # Add optimization prefix,constraint Agent Reply format
        INTERVIEW_PROMPT_PREFIX = (
            "you are undergoing an interview.Please combine it with your personality,All past memories and actions,"
            ...
            "3. Do not use Markdown Title(Such as#,##,###)\n"
            "4. Answer one by one according to question number,Each answer begins with[question X:]Beginning(X Number the question)\n"
            "5. Separate the answers to each question with a blank line\n"
            "6. Answers should have substance,Answer each question at least 2-3 sentence\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"

        # Step 4: call real interview API(Not specified platform,Dual-platform simultaneous interviews by default)
        try:
            # Build batch interview list(Not specified platform,Dual platform interview)
            interviews_request = []
            for agent_idx in selected_indices:
                interviews_request.append({
                    "agent_id": agent_idx,
                    "prompt": optimized_prompt  # Use optimized prompt
                    # Not specified platform,API will be in twitter and reddit Interviews on both platforms
                })

            logger.info(t("console.callingBatchInterviewApi", count=len(interviews_request)))

            # call SimulationRunner batch interview method(Not passed on platform,Dual platform interview)
            api_result = SimulationRunner.interview_agents_batch(
                simulation_id=simulation_id,
                interviews=interviews_request,
                platform=None,  # Not specified platform,Dual platform interview
            ...
            # Check API Is the call successful
            if not api_result.get("success", False):
                error_msg = api_result.get("error", "unknown error")
                logger.warning(t("console.interviewApiReturnedFailure", error=error_msg))
                result.summary = f"interview API call failed:{error_msg}.please check OASIS Simulate environment state."
                return result
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Get the Agent Interview results on both platforms
                ...
                # Clean up possible tool calls JSON package
                ...
                # Clean response text:remove mark,No,Markdown Waiting for interference
                ...
                # Strategy 1(Lord): Extract complete sentences with substantial content
                ...
                # Strategy 2 (fallback): extract text inside paired curly quotes.
                if not key_quotes:
            ...
            result.summary = f"Interview failed:{str(e)}.The simulation environment may be down,please make sure OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        # Try to read first Reddit JSON Format
        ...
        if os.path.exists(twitter_profile_path):
            try:
                with open(twitter_profile_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        # CSV Convert format to unified format
                        profiles.append({
    ...
    def _select_agents_for_interview(
        ...
        """
        use LLM Choose to interview Agent

        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: selected Agent Complete list of information
                - selected_indices: selected Agent index list of(used for API call)
        ...
        system_prompt = """You are a professional interview planning expert.Your task is based on interview needs,from simulation Agent Select the most suitable person to interview from the list.

selection criteria:
1. Agent identity/Careers related to interview topics
2. Agent May hold unique or valuable perspectives
3. Choose a diverse perspective(Such as:Supporters,Opposition,neutral party,Professionals etc)
4. Prioritize roles directly related to the event

Return JSON Format:
{
    "selected_indices": [selected Agent index list of],
    "reasoning": "Reason for selection"
}"""
...
optional Agent list(total{len(agent_summaries)}a):
{json.dumps(agent_summaries, ensure_ascii=False, indent=2)}

Please select the most{max_agents}the most suitable for interviews Agent,and explain the reasons for the choice."""
        ...
        try:
            ...
            # Get selected Agent Complete information
            selected_agents = []
            valid_indices = []
    ...
    def _generate_interview_questions(
        ...
        """use LLM Generate interview questions"""

        agent_roles = [a.get("profession", "unknown") for a in selected_agents]

        system_prompt = """You are a professional journalist/interviewer.According to interview needs,generate 3-5 in-depth interview questions.
...
5. Each problem is controlled within 50 Within words,concise and clear
...
Please generate 3-5 interview questions."""
    ...
    def _generate_interview_summary(
        ...
        system_prompt = f"""You are a professional news editor.Please base your answers on multiple respondents,Generate an interview summary.
...
5. controlled in 1000 within the word

format constraints(Must comply with):
- Use plain text paragraphs,Separate different parts with blank lines
- Do not use Markdown Title(Such as#,##,###)
- Don t use dividing lines(Such as---,***)
- {quote_instruction}
- Can be used**Bold**Mark keywords,but don t use other Markdown Grammar"""
