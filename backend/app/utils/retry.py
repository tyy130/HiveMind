...
for processing LLM Waiting for external API Called retry logic
"""
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"function {func.__name__} in {max_retries} Still failed after retries: {str(e)}")
                        raise
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        logger.error(f"asynchronous function {func.__name__} in {max_retries} Still failed after retries: {str(e)}")
                        raise
...
class RetryableAPIClient:
    """
    retryable API client encapsulation
    """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt == self.max_retries:
                    logger.error(f"API Called at {self.max_retries} Still failed after retries: {str(e)}")
                    raise
                ...
                logger.warning(
                    f"API Call No {attempt + 1} failed attempts: {str(e)}, "
                    f"{current_delay:.1f}Try again in seconds..."
                )
    ...
    def call_batch_with_retry(
        ...
        Args:
            ...
            process_func: Handling function,receive single item as parameter
            exceptions: Exception type that needs to be retried
