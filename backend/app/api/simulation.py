"""
Simulation related API routing
Step2: Zep Entity reading and filtering,OASIS Preparing and running the simulation(Full automation)
"""
...
def _get_default_platform(simulation_id: str) -> str:
    """
    Returns the default platform based on simulation configuration

    read SimulationState in enable_twitter / enable_reddit settings,
    Returns the actual platform used by this simulation,rather than hardcoding 'reddit'.
...
# Interview prompt Optimize prefix
# Adding this prefix avoids Agent Call tool,Reply directly with text
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize Interview Ask a question,Add prefix to avoid Agent Call tool
...
def get_graph_entities(graph_id: str):
    ...
    Only return nodes that match predefined entity types(Labels Not just Entity node)
...
def create_simulation():
    ...
    Note:max_rounds The parameters are given by LLM Intelligent generation,No manual setup required

    Request(JSON):
        {
            "project_id": "proj_xxxx",      // Required
            "graph_id": "mirofish_xxxx",    // Optional,If not provided then project get
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note:run script(run_*.py)remain in backend/scripts/ Directory,No longer copied to mock directory
    ...
    # List of necessary documents(Does not include scripts,The script is located at backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # if config_generated=True and the file exists,Think preparation is complete
        # The following statuses all indicate that the preparation work has been completed:
        # - ready: Ready to complete,Can run
        # - preparing: if config_generated=True Description completed
        # - running: Running,It means that preparations have been completed long ago
        # - completed: Run completed,It means that preparations have been completed long ago
        # - stopped: Stopped,It means that preparations have been completed long ago
        # - failed: Run failed(But the preparation is complete)
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If the status is preparing But the file is complete,The automatic update status is ready
            if status == "preparing":
                try:
            ...
            return False, {
                "reason": f"The status is not in the prepared list or config_generated for false: status={status}, config_generated={config_generated}",
                "status": status,
                "config_generated": config_generated
            }
...
def prepare_simulation():
    """
    Prepare simulation environment(Asynchronous tasks,LLM Intelligent generation of all parameters)
    ...
    Features:
    - Automatic detection of completed preparations,avoid duplicate generation
    - if ready,Directly return existing results
    - Support forced regeneration(force_regenerate=true)

    steps:
    1. Check whether preparations have been completed
    2. from Zep Graph reads and filters entities
    3. generated for each entity OASIS Agent Profile(With retry mechanism)
    4. LLM Intelligent generation of simulation configurations(With retry mechanism)
    5. Save configuration files and preset scripts

    Request(JSON):
        {
            "simulation_id": "sim_xxxx",                   // Required,Simulation ID
            "entity_types": ["Student", "PublicFigure"],  // Optional,Specify entity type
            "use_llm_for_profiles": true,                 // Optional,Whether to use LLM Generate character
            "parallel_profile_count": 5,                  // Optional,Number of characters generated in parallel,Default 5
    ...
    try:
        ...
        # Check if it is ready(avoid duplicate generation)
        if not force_regenerate:
            ...
            if is_prepared:
                logger.info(f"Simulation {simulation_id} Ready,Skip duplicate generation")
                return jsonify({
                    "success": True,
                    "data": {
                ...
                logger.info(f"Simulation {simulation_id} Not ready yet,The preparation task will be started")
        ...
        # In this way the front end is calling prepare You can get the expected results immediately after Agent total
        try:
            logger.info(f"Get the number of entities synchronously: graph_id={state.graph_id}")
            reader = ZepEntityReader()
            # Fast reading of entities(No side information required,Only count the quantity)
            filtered_preview = reader.filter_defined_entities(
                graph_id=state.graph_id,
                defined_entity_types=entity_types_list,
                enrich_with_edges=False  # No side information is obtained,speed up
            )
            # Save entity quantity to state(For front-end to obtain immediately)
            ...
            logger.warning(f"Failed to obtain the number of entities synchronously(Will try again in background task): {e}")
            # Failure does not affect subsequent processes,Background tasks will be reacquired
        ...
        # Update simulation status(Contains the number of entities prefetched)
...
def get_prepare_status():
    ...
    1. Pass task_id Query the progress of ongoing tasks
    2. Pass simulation_id Check whether preparations have been completed

    Request(JSON):
        {
            "task_id": "task_xxxx",          // Optional,prepare returned task_id
            "simulation_id": "sim_xxxx"      // Optional,Simulation ID(Used to check completed preparations)
        }
    ...
    try:
        ...
        # if provided simulation_id,First check whether it is ready
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
        ...
        if not task_id:
            if simulation_id:
                # Yes simulation_id but not ready yet
                return jsonify({
                    "success": True,
                    "data": {
                        "simulation_id": simulation_id,
        ...
        if not task:
            # Task does not exist,But if there is simulation_id,Check if it is ready
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def list_simulations():
    ...
    Query parameters:
        project_id: by project ID filter(Optional)
    """
    try:
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Traverse reports Directory,find out simulation_id matching report,
    If there are multiple return the latest(press created_at sort)
    ...
    # __file__ Yes app/api/simulation.py,Need to go up two levels to backend/
    ...
    try:
        ...
        logger.warning(f"Find simulation {simulation_id} of report failed: {e}")
        return None
...
def get_simulation_history():
    ...
    Used to display historical items on the home page,Returns the project name containing,Simulation list with rich information such as description
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get the file list of associated projects(most 3 a)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulated in real time Agent Profile(Used to view progress in real time during the build process)

    with /profiles The difference between interfaces:
    - Read files directly,not pass SimulationManager
    ...
    try:
        ...
        logger.error(f"Get in real time Profile failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get simulation configuration in real time(Used to view progress in real time during the build process)

    with /config The difference between interfaces:
    - Read files directly,not pass SimulationManager
    - Suitable for real-time viewing during the build process
    - Return additional metadata(Such as file modification time,Whether it is being generated etc)
    ...
    try:
        ...
        logger.error(f"Get in real time Config failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration(LLM Smartly generated complete configuration)

    Return contains:
        - time_config: Time configuration(Simulation duration,rounds,peak/Trough period)
        - agent_configs: each Agent activity configuration(Activity,speaking frequency,stance etc)
        - event_config: event configuration(initial post,hot topics)
        - platform_configs: Platform configuration
        - generation_reasoning: LLM Configuration reasoning description of
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download the simulation run script file(universal script,located in backend/scripts/)

    script_name Optional value:
    ...
    """
    try:
        # The script is located at backend/scripts/ Directory
...
# ============== Profile Generate interface(Standalone use) ==============

@simulation_bp.route('/generate-profiles', methods=['POST'])
def generate_profiles():
    """
    Generate directly from the map OASIS Agent Profile(Do not create a simulation)
...
def start_simulation():
    ...
    Request(JSON):
        {
            ...
            "max_rounds": 100,                     // Optional: Maximum number of simulation rounds,Used to truncate overly long simulations
            "enable_graph_memory_update": false,   // Optional: whether to Agent Activity updates to Zep map memory
            "force": false                         // Optional: force restart(Will stop the running simulation and clear the logs)
        }

    About force parameters:
        - After enabling,If the simulation is running or has completed,It will stop and clean the running log first
        - The cleaning content includes:run_state.json, actions.jsonl, simulation.log Wait
        - Configuration files will not be cleaned(simulation_config.json)and profile File
        - Suitable for scenarios where the simulation needs to be re-run

    About enable_graph_memory_update:
        - After enabling,All in simulation Agent activities(post,Comment,Like etc)will be updated in real time Zep Atlas
        ...
        - Use batch update mechanism,reduce API Number of calls
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if needs_finalization:
                    ...
                    logger.info(f"Forced mode:Complete old simulation termination first {simulation_id}")
                    try:
        ...
        # Get map ID(Used for map memory update)
        graph_id = None
        ...
        with graph_guard:
            if enable_graph_memory_update:
                ...
                logger.info(
                    "Enable map memory updates: simulation_id=%s, graph_id=%s",
                    simulation_id,
                    graph_id,
                )

            # Start simulation.Still held when map writing is enabled graph_guard,until updater
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed status of simulation run(Contains all actions)
...
def get_agent_stats(simulation_id: str):
    ...
    For front-end display Agent Activity ranking,Action distribution etc
    """
    try:
...
def get_simulation_posts(simulation_id: str):
    ...
    Return to post list(from SQLite Database reading)
    """
    try:
        platform = request.args.get('platform') or _get_default_platform(simulation_id)
...
def get_simulation_comments(simulation_id: str):
    ...
    Query parameters:
        platform: platform type(twitter/reddit,Automatic selection based on simulation configuration)
...
def interview_agent():
    ...
    Note:This feature requires the simulation environment to be running(After completing the simulation cycle enter the waiting command mode)
    ...
    Request(JSON):
        {
            ...
            "platform": "twitter",             // Optional,designated platform(twitter/reddit)
    ...
    Return(Not specified platform,Dual platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
    ...
    try:
        ...
        # Optimize prompt,Add prefix to avoid Agent Call tool
...
def interview_agents_batch():
    ...
    Request(JSON):
        {
            ...
            "interviews": [                    // Required,Interview list
                {
                    "agent_id": 0,
                    "prompt": "you are right A What do you think?",
                    "platform": "twitter"      // Optional,Specify the Agent interview platform
                },
                {
                    "agent_id": 1,
                    "prompt": "you are right B What do you think?"  // Not specified platform then use the default value
                }
            ],
            "platform": "reddit",              // Optional,Default platform(by each item platform Cover)
                                               // When not specified:Dual platform simulation for each Agent Interview on two platforms at the same time
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            # Verify each item platform(if there is)
        ...
        # Optimize each interview item s prompt,Add prefix to avoid Agent Call tool
...
def interview_all_agents():
    """
    global interview - Interview all using the same questions Agent
    ...
    Request(JSON):
        {
            "simulation_id": "sim_xxxx",            // Required,Simulation ID
            "prompt": "What is your overall opinion on this matter?",  // Required,interview questions(all Agent Use the same question)
            "platform": "reddit",                   // Optional,designated platform(twitter/reddit)
                                                    // When not specified:Dual platform simulation for each Agent Interview on two platforms at the same time
    ...
    try:
        ...
        logger.error(f"overall situation Interview failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_interview_history():
    ...
    Read all from simulated database Interview record
    ...
    Request(JSON):
        {
            ...
            "agent_id": 0,                 // Optional,Get only the Agent interview history
    ...
    try:
        ...
        logger.error(f"get Interview historical failure: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
    ...
    Return:
        {
            ...
            "data": {
                ...
                "message": "environment is running,Can receive Interview command"
            }
        }
    """
    try:
